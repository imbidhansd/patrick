...
class ProfessionalAffiliationController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $data = [
            ...
            'search' => [
                'professional_affiliations.trade_id' => [
                    'title' => 'Trade',
    ...
    public function reorder($trade_id, Request $request) {
        $item_list = $this->modelObj->order()->get();
        if ($trade_id > 0) {
            $item_list = $this->modelObj->where('trade_id', $trade_id)->order()->get();
        }
        
        $data = [
            'admin_page_title' => 'Reorder' . $this->module_plural_name,
            'trades' => $this->common_data['trades'],
            'trade_id' => $trade_id,
            'item_list' => $item_list
        ];
