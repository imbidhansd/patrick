...
<div class="text-center">
    <div class="row">
        <div class="col-md-4 align-self-center">
            ...
            <div class="company_logo profile_logo_border font-bold font-18">
                <p>{{ $companyObj->company_name }}</p>
            </div>
            ...
            <div class="company_address" id="company_address_display" style="{{ $show }}">
                ...
                <span>
                    ...
                    if ($companyObj->company_mailing_address != '') {
                        ...
                        $address_arr_line_1[] = $companyObj->company_mailing_address;
                    }

                    if ($companyObj->suite != '') {
                        $address_arr[] = $companyObj->suite;
                        $address_arr_line_1[] = $companyObj->suite;
                    }

                    if ($companyObj->city != '') {
                        $address_arr[] = $companyObj->city;
                        $address_arr_line_2[] = $companyObj->city;
                    }

                    if (!is_null($companyObj->state) && $companyObj->state->name != '') {
                        $address_arr[] = $companyObj->state->name;
                        $address_arr_line_2[] = $companyObj->state->name;
                    }

                    if ($companyObj->zipcode != '') {
                        $address_arr[] = $companyObj->zipcode;
                        $address_arr_line_2[] = $companyObj->zipcode;
                    }
        ...
        <div class="col-md-4 align-self-center">
            <div class="rattings">
                <h6 class="avg_rating">Average Rating </h6>

                <div class="text-center">
                    <div class="starHalf"></div>
                    <br />
                    <a href="javascript:;" data-href="#customer_reviews" class="scroll_btn review_btn font-weight-normal font-14">{{ ((!is_null($average_ratings)) ? $average_ratings->total_reviews : 0) }} Reviews</a>
                </div>
            </div>
...
<div class="profile_sub_menu">
    ...
    <ul class="nav justify-content-center">
        ...
        <li class="nav-item">
            <a href="javascript:;" data-href="javascript:;" class="nav-link scroll_btn" id="create_request">Contact {{ $companyObj->showCompanyName() }} <?php /* 's */ ?></a>
        </li>
    </ul>
