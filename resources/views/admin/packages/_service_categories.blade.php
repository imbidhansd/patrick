            ...
            <table class="table table-hover table-striped">
                <thead>
                    <tr>
                        <th>
                            <div class="checkbox checkbox-primary">
                                <input value="main_category_id[]" class="main_category_item" value="{{ $main_category_item['main_category_id'] }}" id="step_2_main_category_{{ $type_item['service_category_type_id'] }}_{{ $main_category_item['main_category_id'] }}" type="checkbox"  />
                                <?php /* class="chk_all last_input" */ ?>
                        ...
                        <th>
                            @if (isset($package_membership_type) && ($package_membership_type == '4' ||
                            $package_membership_type == '5'))
                    ...
                    <tr class="service_category_item_list">
                        ...
                        @php
                        if(isset($package_id) && $package_id != ''){
                        $package_service_category_fee = \App\Models\PackageServiceCategory::where([['service_category_id',
                        $item->id], ['package_id', $package_id]])->first();
                        }
                        ...
                        <td>
                            <div class="checkbox checkbox-primary">
                                <input name="service_category_ids[]" value="{{ $item->id }}" id="step_2_service_category_{{ $item->id }}" type="checkbox" class="chk_service_category_id_step_2 last_input" />
                                <label for="step_2_service_category_{{ $item->id }}">{{ $item->title }}</label>
                            </div>
                        </td>
                        <td>
                            @if (isset($package_membership_type) && $package_membership_type == '6')
                            <div class="form-group mb-0">
                                <input type="text"
                                       name="service_category_fee[{{ $type_item['service_category_type_id'] }}][{{ $item->id }}]" value="" data-val="{{ number_format($fee, 2, '.', '') }}" class="form-control service_category_fee text-right" disabled />
                            </div>
                            @endif
                        </td>
                    </tr>
