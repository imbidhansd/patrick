        ...
        <table class="table table-hover">
                ...
                <tr>
                    <th>
                        <div class="checkbox checkbox-primary">
                            ...
                            <label for="chk_all">
                                {!! \App\Models\Custom::getSortingLink($module_urls['list'], 'Question', $url_key. '.question',
                        ...
                        <th>
                            {!! \App\Models\Custom::getSortingLink($module_urls['list'], 'Company', 'companies.company_name',
                                $list_params['sort_by'], $list_params['sort_order'], $list_params['search_field'],
                                $list_params['search_text'], http_build_query($list_params)) !!}
                        </th>
                        <th>
                            {!! \App\Models\Custom::getSortingLink($module_urls['list'], 'User', 'company_users.first_name',
                                $list_params['sort_by'], $list_params['sort_order'], $list_params['search_field'],
                                $list_params['search_text'], http_build_query($list_params)) !!}
                        </th>
                        <th>
                            {!! \App\Models\Custom::getSortingLink($module_urls['list'], 'Date & Time', $url_key. '.created_at',
                                $list_params['sort_by'], $list_params['sort_order'], $list_params['search_field'],
                                $list_params['search_text'], http_build_query($list_params)) !!}
                            </th>
                            <th>
                                {!! \App\Models\Custom::getSortingLink($module_urls['list'], 'Status', $url_key. '.status',
                                $list_params['sort_by'], $list_params['sort_order'], $list_params['search_field'],
                                $list_params['search_text'], http_build_query($list_params)) !!}
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $row->company_user->first_name }} {{ $row->company_user->last_name }}</td>
                            <td>{{ $row->created_at->format(env('DATETIME_FORMAT', 'm/d/Y H:i:s')) }}</td>
                            <td>
                                @if ($row->status == 'new')
            ...
            <div class="row">
                <div class="col-sm-12 col-md-6">
                    @include ('admin.includes._tfoot', ['options' => ['new' => 'New', 'pending' => 'Pending', 'resolved' => 'Resolved', 'delete' => 'Delete']])
                </div>
                <div class="col-sm-12 col-md-6">
