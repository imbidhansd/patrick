        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $row->size}}</td>
                    <td>{{ $row->banner_for}}</td>
                    <td>
                        @if (!is_null($row->banner_url))
                        <a data-toggle="tooltip" title="View Banner" class="btn btn-xs btn-info" href="{{ $row->banner_url }}" data-fancybox="gallery_{{ $row->id }}">
                            <i class="fas fa-eye"></i>
                        </a>
                        @endif
                        
                        <?php /*
                        @if ($row->media->file_name != '')
                        <a href="{{ asset('/')}}uploads/media/{{ $row->media->file_name}}" data-fancybox="gallery">
                            <img src="{{ asset('/')}}uploads/media/fit_thumbs/50x50/{{ $row->media->file_name}}" class="img-thumbnail" />
                        </a>
                        @endif
                        */ ?>
                    </td>
