...
@include('admin.includes.searchForm', ['search' => $search])
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th>Trade</th>
                    <th>
                        {!! \App\Models\Custom::getSortingLink($module_urls['list'], '#TLC Id', $url_key. '.tlc_id',
                        $list_params['sort_by'], $list_params['sort_order'], $list_params['search_field'],
                        $list_params['search_text'], http_build_query($list_params)) !!}
                    </th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if (!is_null($row->top_level_category_trades))
                        @foreach ($row->top_level_category_trades as $trade_item)
                        {!! $trade_item->trade->title !!}<br />
                        @endforeach
                        @endif
                    </td>
                    <td>{{ $row->tlc_id }}</td>
